...
void Viewer::init() {
  ...
  // create window
  string title = renderer ? renderer->name() : "Viewer";
  ...
  line_id_framerate = osd_text->add_line(-0.98, -0.96, "", 
                                          14, Color(0, 0, 0));
...
void Viewer::drawInfo() {
  ...
  if (elapsed >= 1.0f) {
    ...
    Color c = framecount < 30 ? Color(1.0, 0.0, 0.0) : Color(0.0, 1.0, 0.5);
    osd_text->set_color(line_id_framerate, c);
    string framerate_info = to_string(framecount) + " fps";
